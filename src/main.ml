...
let select =
  El.select
    (List.map
       (fun v ->
         El.option
           ~at:[ At.value (Jstr.v @@ Ocamlopt.dump_to_string v) ]
           [ El.txt' (Ocamlopt.dump_to_string v) ])
       Ocamlopt.all_dump)
...
let dump_output = get_el_by_id "dump"
let asm_output = get_el_by_id "asm"
...
let setup () =
  ...
  let on_click _ =
    let run () =
      ...
      let document = Edit.get_doc view ^ ";;" in
      let dump, asm =
        try
          Ocamlopt.dump_program document
            [
              Jv.get (El.to_jv select) "value"
              |> Jv.to_string |> Ocamlopt.string_to_dump |> Option.get;
            ]
        with exn ->
          Brr.Console.warn [ Jstr.v @@ Printexc.to_string exn ];
          ("", "")
      in
      El.set_children dump_output
        [
          El.p
            ~at:[ At.v (Jstr.v "style") (Jstr.v "white-space: pre-wrap;") ]
            [ El.txt' dump ];
        ];
      El.set_children asm_output
        [
          El.p
            ~at:[ At.v (Jstr.v "style") (Jstr.v "white-space: pre-wrap;") ]
            [ El.txt' asm ];
        ];
      let* o = with_rpc rpc Toprpc.exec document in
...
let () =
  let sel = get_el_by_id "select-holder" in
  El.set_children sel [ select ];
  Cmi_for_ocamlopt.run ();
  async_raise setup
